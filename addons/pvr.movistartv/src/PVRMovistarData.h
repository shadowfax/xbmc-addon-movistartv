...
struct PVRMovistarChannel
{
  ...
  std::vector<PVRMovistarEpgEntry> epg;
};

struct PVRMovistarRecording
{
  int         iDuration;
  int         iGenreType;
  int         iGenreSubType;
...
struct PVRMovistarChannelGroup
{
  bool             bRadio;
  int              iGroupId;
...
class PVRMovistarData
{
public:
  PVRMovistarData(void);
  virtual ~PVRMovistarData(void);

  virtual int GetChannelsAmount(void);
  virtual PVR_ERROR GetChannels(ADDON_HANDLE handle, bool bRadio);
  virtual bool GetChannel(const PVR_CHANNEL &channel, PVRMovistarChannel &myChannel);
...
private:
  ...
  std::vector<PVRMovistarRecording>    m_recordings;
  std::vector<PVRMovistarTimer>        m_timers;
  time_t                           m_iEpgStart;
